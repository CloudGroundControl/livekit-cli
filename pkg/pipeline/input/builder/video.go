...
func NewSDKVideoInput(p *config.PipelineConfig, src *app.Source, codec webrtc.RTPCodecParameters) (*VideoInput, error) {
	...
	if p.OutputJpeg {
		if err := v.buildJpegEncoder(p); err != nil {
			return nil, err
		}
	} else {
...
func (v *VideoInput) buildSDKDecoder(p *config.PipelineConfig, src *app.Source, codec webrtc.RTPCodecParameters) error {
	...
	*/
	...
	v.elements = append(v.elements, videoQueue, videoConvert, videoScale, caps)
...
func (v *VideoInput) buildJpegEncoder(p *config.PipelineConfig) error {
	jpegEnc, err := gst.NewElement("jpegenc")
	if err != nil {
		return err
	}
	/*
		if err = jpegEnc.SetProperty("quality", 30); err != nil {
			return err
		}
	*/
	v.elements = append(v.elements, jpegEnc)
